...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`web-audit-start-rnd: Authenticated user ${userId}`);
    ...
    const proxyUrl = Deno.env.get('STAGING_CLOUD_RUN_PROXY_URL');
    if (proxyUrl) {
      const complexityPromises = scopeFileStats.map(async (file) => {
        try {
          ...
          console.warn(`web-audit-start-rnd: Complexity estimation failed for ${file.path}:`, err);
        }
      });
    ...
    console.log(`web-audit-start-rnd: Scope ${scopeNloc} nLOC, context ${contextNloc} nLOC, cost ${estimatedCost} credits`);
    ...
    if (existingAudits && existingAudits.length > 0) {
      console.warn(`web-audit-start-rnd: Conflict – user ${userId} already has active session ${existingAudits[0].session_id} (status: ${existingAudits[0].status})`);
    ...
    const { data: deductResult, error: deductError } = await supabase.rpc('cli_deduct_credits', {
      ...
      p_description: `Web audit (staging): ${projectName} (${scopeFileStats.length} contracts)`,
    });
    ...
    if (auditError || !audit) {
      console.error('web-audit-start-rnd: Failed to create audit:', auditError);
    ...
    if (orchError) {
      console.error('web-audit-start-rnd: Failed to create orchestration row:', orchError);
    ...
    if (!proxyUrl || !sessionSecret) {
      throw new Error('STAGING_CLOUD_RUN_PROXY_URL or SESSION_SECRET not configured');
    }
    ...
    console.error('web-audit-start-rnd: Unexpected error:', errorMessage);

    if (creditsDeducted && deductedUserId) {
      try {
        await supabase.rpc('cli_refund_credits', { p_user_id: deductedUserId, p_amount: deductedAmount, p_audit_id: createdSessionId, p_description: 'Refund: Unexpected error in web audit start (staging)' });
      } catch (cleanupErr) { console.error('web-audit-start-rnd: Failed to refund credits:', cleanupErr); }
    }
    if (createdSessionId) {
      try {
      ...
      } catch (cleanupErr) { console.error('web-audit-start-rnd: Failed to delete orphan audit/orchestration:', cleanupErr); }
    }
