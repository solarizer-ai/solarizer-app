...
const ScanProgressWidget = ({ isVisible, projectName, findings, auditStatus, onCancel, onViewResults, onClose }: ScanProgressWidgetProps) => {
      ...
      <div className="bg-card border border-border rounded-lg shadow-xl overflow-hidden">
        <div className="flex items-center justify-between gap-3 px-3 py-2 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setIsExpanded(!isExpanded)}>
          <div className="flex items-center gap-2 min-w-0">
            ...
            {isExpanded ? <span className="text-sm font-medium text-foreground truncate">{isScanning ? "Analyzing..." : "Analysis Complete"}</span> : (
              <div className="flex items-center gap-2"><span className="text-sm font-medium text-foreground">{isScanning ? "Scanning..." : "Complete"}</span>{totalFindings > 0 && <Badge variant="secondary" className="text-xs">{totalFindings}</Badge>}</div>
            )}
          </div>
